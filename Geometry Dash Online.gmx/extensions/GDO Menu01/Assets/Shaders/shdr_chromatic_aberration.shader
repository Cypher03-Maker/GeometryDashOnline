...
void main()
{
    ...
    tex_color.r = texture2D(gm_BaseTexture, vec2(uv.x + displacementX, uv.y + displacementY)).x;
    tex_color.g = texture2D(gm_BaseTexture, vec2(uv.x, uv.y)).y;
    tex_color.b = texture2D(gm_BaseTexture, vec2(uv.x - displacementX, uv.y - displacementY)).z;
